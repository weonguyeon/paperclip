  ...
  return (
    <div className={cn("relative", cards && "space-y-6")}>
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  프롬프트 템플릿은 매 하트비트마다 재실행됩니다. 반복적인 토큰 비용과 캐시 낭비를 줄이려면 간결하고 동적으로 유지하세요.
                </div>
              </>
            )}
          </div>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label="작업 디렉토리 (사용 중단됨)" hint={help.cwd}>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                프롬프트 템플릿은 매 하트비트마다 재실행됩니다. <code>{"{{ context.* }}"}</code>나 <code>{"{{ run.* }}"}</code> 같은 변수를 활용한 간결한 작업 서술을 권장하며, 변하지 않는 지시사항은 여기에 반복하지 마세요.
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">권한 및 구성</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">권한 및 구성</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              <ModelDropdown
                ...
                detectModelLabel="모델 감지"
                emptyDetectHint="감지된 모델 없음. 수동으로 선택하거나 입력하세요."
              />
              {fetchedModelsError && (
                ...
                <>
                  <Field label="부트스트랩 프롬프트 (레거시)" hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      ...
                      placeholder="첫 번째 실행을 위한 선택적 초기 설정 프롬프트"
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    부트스트랩 프롬프트는 레거시이며 향후 릴리스에서 제거될 예정입니다. 이 내용을 에이전트의 프롬프트 템플릿 또는 지시사항 파일로 이동하는 것을 고려하세요.
                  </div>
                </>
              )}
              ...
              {!isCreate && (
                <>
                  ...
                  <Field label="중단 유예 시간 (초)" hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <div className="space-y-3">
              ...
              />
              <Field label="재실행 대기 시간 (초)" hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? "모델 검색... (입력하여 직접 추가)" : "모델 검색..."}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? "감지 중..." : detectedModel ? (detectModelLabel?.replace(/^모델 감지$/, "모델 재감지") ?? "설정에서 재감지") : (detectModelLabel ?? "설정에서 감지")}
            </button>
          )}
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? "감지된 모델이 없습니다. 공급자/모델을 직접 입력하세요.")
                    : "모델을 찾을 수 없습니다."}
                </p>
              </div>
            )}
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label="사고 강도" hint={help.thinkingEffort}>
