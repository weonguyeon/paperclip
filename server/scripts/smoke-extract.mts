...
const files = readdirSync(samplesDir).filter((n) => !n.startsWith("_"));
for (const name of files) {
  const buf = readFileSync(join(samplesDir, name));
  const ct = name.endsWith(".pdf")
    ? "application/pdf"
    : name.endsWith(".png")
    ? "image/png"
    : name.endsWith(".csv")
  ...
  const preview = (result.text ?? "").slice(0, 160).replace(/\s+/g, " ");
