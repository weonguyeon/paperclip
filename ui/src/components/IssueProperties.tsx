...
export function IssueProperties({
  ...
  const executionParticipantLabel = (value: string) => {
    ...
    if (value.startsWith("user:")) {
      return userLabel(value.slice("user:".length)) ?? "사용자";
    }
    return value;
  };
  ...
  const runExecutionButton = (stageType: "review" | "approval") => (
    <PropertyRow label="">
      ...
      >
        {stageType === "review" ? "지금 검토 실행" : "지금 승인 실행"}
      </button>
    </PropertyRow>
  );
  const currentExecutionLabel = (() => {
    if (!issue.executionState?.currentStageType) return null;
    const stageLabel = issue.executionState.currentStageType === "review" ? "검토" : "승인";
    ...
    if (issue.executionState.status === "changes_requested") {
      return `${stageLabel} 변경 요청됨${participantLabel ? ` (${participantLabel})` : ""}`;
    }
    return `${stageLabel} 대기 중${participantLabel ? ` (${participantLabel})` : ""}`;
  })();
  ...
  const labelsTrigger = (issue.labels ?? []).length > 0 ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">레이블 없음</span>
    </>
  );
  ...
  const labelsContent = (
    <>
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        ...
        >
          <Plus className="h-3 w-3" />
          {createLabel.isPending ? "생성 중…" : "레이블 생성"}
        </button>
      </div>
    </>
  );
  ...
  const executionParticipantsContent = (
    ...
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={`${stageType === "review" ? "검토자" : "승인자"} 검색...`}
        value={search}
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        <button
          ...
          onClick={onClear}
        >
          {stageType === "review" ? "검토자 없음" : "승인자 없음"}
        </button>
        {currentUserId && (
          <button
            className={cn(
        ...
        {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
          ...
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
            {creatorUserLabel ? creatorUserLabel : "요청자"}
          </button>
        )}
        {sortedAgents
          .filter((agent) => {
  ...
  const parentTrigger = issue.parentId ? (
    ...
    <span className="text-sm text-muted-foreground">상위 이슈 없음</span>
  );
  ...
  const blockedByTrigger = blockedByIds.length > 0 ? (
    ...
    <span className="text-sm text-muted-foreground">차단 없음</span>
  );
