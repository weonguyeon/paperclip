...
async function readLocalPackageZip(file: File): Promise<{
  ...
  if (!/\.zip$/i.test(file.name)) {
    throw new Error(".zip 회사 패키지를 선택하세요.");
  }
  const archive = await readZipArchive(await file.arrayBuffer());
  if (Object.keys(archive.files).length === 0) {
    throw new Error("선택한 zip 아카이브에서 패키지 파일을 찾을 수 없습니다.");
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const localZipHelpText =
    "Paperclip에서 직접 내보낸 .zip 파일을 업로드하세요. Finder, 탐색기 등 다른 압축 도구로 재압축한 파일은 올바르게 가져오지 못할 수 있습니다.";
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error("소스가 설정되지 않았습니다.");
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: "미리보기 실패",
        body: err instanceof Error ? err.message : "가져오기 미리보기에 실패했습니다.",
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: "가져오기 완료",
        body: `${result.company.name}: 에이전트 ${result.agents.length}개 처리됨.`,
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: "가져오기 실패",
        body: err instanceof Error ? err.message : "가져오기 적용에 실패했습니다.",
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: "패키지 읽기 실패",
        body: err instanceof Error ? err.message : "폴더를 읽는 데 실패했습니다.",
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message="가져올 회사를 선택하세요." />;
  }
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {localPackage.name} — 파일 {Object.keys(localPackage.files).length}개
                </span>
              )}
            </div>
            {!localPackage && (
          ...
          <Field
            label="GitHub URL"
            hint="COMPANY.md의 저장소 트리 경로 또는 blob URL (예: github.com/owner/repo/tree/main/company)."
          >
            <input
        ...
        <Field label="대상" hint="이 회사로 가져오거나 새 회사를 생성합니다.">
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">새 회사 생성</option>
            <option value="existing">
              기존 회사: {selectedCompany?.name}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label="새 회사 이름"
            hint="선택 입력. 비워 두면 패키지 이름을 사용합니다."
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
        ...
        <Field
          label="충돌 처리 방식"
          hint="일치하는 회사 콘텐츠를 이름 변경, 건너뛰기, 또는 교체할 수 있습니다."
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">충돌 시 이름 변경</option>
            <option value="skip">충돌 시 건너뛰기</option>
            <option value="replace">기존 항목 교체</option>
          </select>
        </Field>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending ? "미리보는 중..." : "가져오기 미리보기"}
          </Button>
        </div>
      </div>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  충돌 {conflicts.length}개
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  오류 {importPreview.errors.length}개
                </span>
              )}
            </div>
          </div>
