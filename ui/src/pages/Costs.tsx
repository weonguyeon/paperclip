  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        ...
        <CardDescription>
          단일 추론 요청에 매핑되지 않는 계정 수준의 청구 내역입니다.
        </CardDescription>
      </CardHeader>
            ...
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `에이전트 ${budgetData?.pausedAgentCount ?? 0}개 일시정지 · 프로젝트 ${budgetData?.pausedProjectCount ?? 0}개 일시정지`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>선택 기간에 각 에이전트가 소비한 비용.</CardDescription>
                  </CardHeader>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>프로젝트 연결 이슈를 통해 귀속된 실행 비용.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage="아직 재무 이벤트가 없습니다. 청구자 인보이스나 크레딧이 도착하면 계정 수준 청구 내역을 추가하세요." />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">예산 제어판</CardTitle>
                  <CardDescription>
                    에이전트와 프로젝트의 강제 지출 한도. 공급자 구독 할당량은 별도로 공급자 탭에 표시됩니다.
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    ...
                    subtitle="미결 소프트/하드 임계값 초과"
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label="승인 대기 중"
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle="이사회 조치 대기 중인 예산 초과 승인"
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label="일시정지된 에이전트"
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      예산을 올리거나 범위를 명시적으로 일시정지 상태로 유지하여 강제 중단을 해결하세요.
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      아직 예산 정책이 없습니다. 에이전트 및 프로젝트 상세 페이지에서 예산을 설정하거나 기존 회사 월별 예산 컨트롤을 사용하세요.
                    </CardContent>
                  </Card>
                ) : null}
              </div>
